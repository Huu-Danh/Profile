...
function GridBackground({
  ...
  return (
    <motion.div
      ...
      className={`relative size-full overflow-hidden ${background}, ${className}`}
      style={gridStyle}
      {...props}
    >
      {/* Grid Container */}
      <div
        className={`absolute inset-0 size-full ${borderColor}`}
        style={{
          display: 'grid',
      ...
      {animatedBeams.map((beam) => {
        ...
        return (
          <motion.div
            key={beam.id}
            className={`absolute rounded-full backdrop-blur-sm z-20${beam.color}
...
function GradientBackground({
  ...
  return (
    <motion.div
      ...
      className={` size-full bg-gradient-to-br from-fuchsia-400 from-0% via-50% via-violet-500 to-fuchsia-600 to-100% bg-[length:300%_300%] 
        ${className}`}
      animate={{
